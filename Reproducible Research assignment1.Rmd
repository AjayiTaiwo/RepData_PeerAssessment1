...
Data1 <- "repdata-data-activity.zip"

if (!file.exists("Data/activity.csv")) {
    dataURL <- "https://d396qusza40orc.cloudfront.net/repdata%2Fdata%2Factivity.zip"
    download.file(dataURL, Data1, mode = "wb")
    unzip(Data1, files = NULL, list = FALSE, overwrite = TRUE, junkpaths = FALSE, exdir = "Data", unzip = "internal", setTimes = FALSE)
    file.remove(Data1)
}
...
CALCULATING THE TOTAL NUMBER OF STEPS TAKEN PER DAY WITH HISTOGRAM
...
plot(sum.steps.day$date, sum.steps.day$steps, type = "h", main = "Histogram of daily steps", xlab = "Date", ylab = "Steps per day", col = "yellow", lwd = 7)
...
```{r}
# Mean steps per Day
paste("The Mean steps taken per Day is  =", round(mean(sum.steps.day$steps), 0))

# Median steps per Day
paste(" The Median steps taken per Day is =", round(median(sum.steps.day$steps), 0))
```
...
plot(mean.steps.interval$interval, mean.steps.interval$steps, type = "l", main = "Average daily activity by interval", xlab = "Interval",
     ylab = "Steps per interval", col = "yellow", lwd = 2)
